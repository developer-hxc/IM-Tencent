...
class TX implements Base
{
    ...
    {
        $api = new TLSSigAPI();
        $api->SetAppid($this->config['appid']);//设置在腾讯云申请的appid
        $private_key = file_get_contents(ROOT_PATH.$this->config['path'].'/private_key.txt');
        $public_key = file_get_contents(ROOT_PATH.$this->config['path'].'/public_key.txt');
